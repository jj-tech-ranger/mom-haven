...
async function startServer() {
  ...
  // Security headers with AI Studio iframe compatibility
  app.use(helmet({
    contentSecurityPolicy: false, // Vite injects dev client scripts; handled cleanly
  ...
  // Clinical & Admin API routes
  app.use('/api/v1/clinician', clinicianRouter);
  app.use('/api/v1/admin', adminRouter);

  // Haven AI Chat config & routes
  ...
  async function verifyIdToken(idToken: string) {
    ...
    if (!r.ok) throw new Error('Invalid Firebase session');
    const d = await r.json();
    const u = d.users?.[0];
    if (!u?.localId) throw new Error('Invalid Firebase session');
  ...
  async function runQuery(idToken: string, structuredQuery: any) {
    ...
    return rows.filter((x: any) => x.document).map((x: any) => ({ id: x.document.name.split('/').pop(), ...documentToObject(x.document) }));
  }
  ...
  async function getDocument(idToken: string, path: string) {
    ...
    return documentToObject(await r.json());
  }

  async function writeDocument(idToken: string, path: string, fields: Record<string, unknown>) {
    const r = await fetch(`${FIRESTORE_BASE}/${path}`, {
      method: 'POST',
      headers: { authorization: `Bearer ${idToken}`, 'content-type': 'application/json' },
  ...
  async function patchDocument(idToken: string, path: string, fields: Record<string, unknown>) {
    ...
    const r = await fetch(`${FIRESTORE_BASE}/${path}?${masks}`, {
      method: 'PATCH',
      headers: { authorization: `Bearer ${idToken}`, 'content-type': 'application/json' },
      body: JSON.stringify({ fields: Object.fromEntries(Object.entries(fields).map(([k, v]) => [k, firestoreValue(v)])) }),
    });
  ...
  async function resolveContext(idToken: string, uid: string) {
    const eq = (field: string) => ({ fieldFilter: { field: { fieldPath: field }, op: 'EQUAL', value: { stringValue: uid } } });
    const [pregnancies, children] = await Promise.all([
      runQuery(idToken, { from: [{ collectionId: 'pregnancies' }], where: eq('motherId'), limit: 25 }),
      runQuery(idToken, { from: [{ collectionId: 'children' }], where: eq('motherId'), limit: 25 }),
    ]);
    const p = pregnancies.find((x: any) => x.status === 'active');
    if (p) return 'This mother has an active pregnancy on record.';
    if (children[0]) return 'This mother has a child on record.';
  ...
  app.post('/api/v1/chat', async (req, res) => {
    try {
      ...
      if (layer === 'physical_danger') return res.json({ classification: 'emergency', responseText: '', suggestedFollowups: [], handoff: 'physical_danger' });
      if (layer === 'self_harm_or_violence') return res.json({ classification: 'emergency', responseText: '', suggestedFollowups: [], handoff: 'self_harm_or_violence' });
      ...
      console.error('Haven chat error', e);
      res.status(e?.message === 'Invalid Firebase session' ? 401 : 500).json({ error: e?.message || 'Unable to reach Haven.' });
    }
  });
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`MomHaven server running on http://0.0.0.0:${PORT}`);
  });
}

startServer().catch(err => {
  console.error('Failed to start MomHaven server:', err);
  process.exit(1);
});
