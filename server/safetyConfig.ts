...
import {DANGER_SIGNS,SELF_HARM_OR_VIOLENCE_PATTERNS,DANGER_SIGN_PATTERNS} from '../shared/safety/dangerSignTriage.js';

let cache:{at:number;version:number;physical:RegExp[];selfHarm:RegExp[]}|null=null;
const TTL=5*60*1000;
const toRegex=(patterns:string[])=>patterns.map(p=>new RegExp(`\\b${p.replace(/[.*+?^${}()|[\]\\]/g,'\\$&').replace(/\\s+/g,'\\s+')}\\b`,'i'));
...
export async function getSafetyPatternConfig(){
 ...
 try{
  ...
  const physical:string[]=[];const selfHarm:string[]=[];let version=1;
  ...
  cache={at:Date.now(),version:0,physical:DANGER_SIGN_PATTERNS.filter(p=>!SELF_HARM_OR_VIOLENCE_PATTERNS.includes(p)),selfHarm:SELF_HARM_OR_VIOLENCE_PATTERNS};return cache;
 }
}

export async function classifyLayerOneRemote(message:string){const normalized=message.replace(/\s+/g,' ').trim();const cfg=await getSafetyPatternConfig();if(cfg.selfHarm.some(p=>p.test(normalized)))return'self_harm_or_violence' as const;if(cfg.physical.some(p=>p.test(normalized)))return'physical_danger' as const;return null}
