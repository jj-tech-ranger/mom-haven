export const sw = {
  common: {
    appName: 'MomHaven',
    appTagline: 'Kila Mama, Kila Mtoto, Kila Hatua ya Maisha.',
    mohTagline: 'Mwenzi wa Kidijitali wa MOH 216',
    previewApp: 'Gundua',
    signIn: 'Ingia',
    signOut: 'Toka',
    createAccount: 'Fungua akaunti',
    createFreeAccount: 'Fungua Akaunti ya Bure',
    exploreMomHaven: 'Gundua MomHaven',
    back: 'Rudi',
    cancel: 'Ghairi',
    save: 'Hifadhi',
    close: 'Funga',
    continue: 'Endelea',
    processing: 'Inachakata...',
    or: 'au',
    loading: 'Inapakia...',
  },

  header: {
    tagline: 'Mwenzi wa Kidijitali wa MOH 216',
    knowledge: 'Maktaba ya Maarifa',
    haven: 'Haven',
    lifecycle: 'Jinsi Inavyofanya Kazi',
    about: 'Kuhusu MomHaven',
    partners: 'Kwa Wenza',
    signIn: 'Ingia',
    explore: 'Gundua MomHaven',
    themeToggleAria: 'Badilisha mandhari ya rangi',
    languageToggleAria: 'Badili lugha hadi Kiingereza',
  },

  hero: {
    badge: 'Kila Mama, Kila Mtoto, Kila Hatua ya Maisha.',
    title: 'Mwongozo thabiti, majibu ya kuaminika, na kumbukumbu salama za mama na mtoto.',
    subtitle: 'Kila Mama, Kila Mtoto, Kila Hatua ya Maisha.',
    description: 'Kuanzia kliniki yako ya kwanza ya ujauzito hadi hatua za ukuaji wa mtoto, MomHaven huweka huduma yako kwa mpangilio, maswali yako yamejibiwa, na familia yako ikiwa na msaada.',
    primaryCta: 'Gundua MomHaven',
    guestPreviewCta: 'Tazama kama Mgeni',
    secondaryCta: 'Ingia',
    createAccountPrompt: 'Huna akaunti bado?',
    createAccountLink: 'Fungua akaunti',
    uiPreview: {
      gestationBadge: 'Wiki ya 24 • Robo ya 2',
      nextAnc: 'Kliniki Inayofuata: St. Mary • Alhamisi 3:00 Asubuhi',
      dailyTip: 'Ushauri wa Haven: Kunywa vidonge vya IFAS na juisi ya machungwa kuongeza ufyonzwaji wa madini ya chuma.',
      havenQuery: 'Je, mgandamizo wa chini ya tumbo katika wiki ya 24 ni kawaida?',
      havenReply: 'Mgandamizo mdogo wa chini ya tumbo hutokea mtoto anapokua. Ikiwa una maumivu makali, maji kutoka, au damu, nenda kituo cha afya mara moja.',
      clinicalSource: 'Kitabu cha Afya ya Mama na Mtoto (MOH Kenya)',
      vitalsBadge: 'Shinikizo la Damu: 118/76 mmHg • Kawaida',
    },
  },

  valueProps: {
    heading: 'Safari yako, ikiungwa mkono katika sehemu moja.',
    subheading: 'Mwongozo thabiti na rekodi endelevu zilizobuniwa kwa ajili ya akina mama, familia, na watoa huduma za afya.',
    learn: {
      title: 'Jifunze',
      desc: 'Taarifa za kuaminika kuhusu afya ya uzazi na mtoto, zilizotungwa kukusaidia kuelewa mambo muhimu.',
      action: 'Soma miongozo ya afya',
      sampleTag: 'Mwongozo wa Robo ya 1',
      sampleHeadline: 'Lishe & Vidonge vya Madini ya Chuma (IFAS)',
      sampleTime: 'Dakika 3 kusoma • Kenya MOH 216',
    },
    track: {
      title: 'Fuatilia',
      desc: 'Weka rekodi zako za ujauzito, kliniki ya wajawazito (ANC), chanjo, na ukuaji wa mtoto kwa mpangilio mzuri.',
      action: 'Angalia rekodi za MOH',
      sampleTag: 'Kadi ya Rekodi za MOH 216',
      sampleVitals: 'BP 118/76 • Uzito 64.5kg • Urefu wa Tumbo 24cm',
      sampleAnc: 'Ziara ya 3 kati ya 8 Imekamilika',
    },
    askHaven: {
      title: 'Muulize Haven',
      desc: 'Pata majibu yenye manufaa kuhusu maswali ya afya na utambue wakati ambapo msaada wa daktari unahitajika.',
      action: 'Uliza swali lako',
      sampleQuery: 'Chanjo ya kwanza ya KEPI inatolewa lini?',
      sampleReply: 'Chanjo za BCG na OPV 0 hutolewa mara baada ya kuzaliwa. Rotavirus na Pentavalent huanza wiki ya 6.',
    },
    stayConnected: {
      title: 'Ungana na Wengine',
      desc: 'Weka taarifa zako muhimu tayari kwa daktari wako na washirikishe watu wanaokusaidia kwenye safari yako.',
      action: 'Shiriki na Mwenzi & Daktari',
      samplePartner: 'Mwenzi Ameunganishwa: David',
      sampleSync: 'Tarehe za kliniki & mawasiliano ya dharura yamesawazishwa',
    },
  },

  lifecycle: {
    badge: 'Mzunguko Kamili wa Huduma za Afya',
    heading: 'Kutoka ujauzito hadi mtoto anapokua.',
    subheading: 'MomHaven si kikokotoo cha kawaida cha ujauzito tu. Ni mwenzi kamili wa afya ya mama na mtoto katika kila hatua muhimu.',
    motherRole: 'Huduma ya Mama',
    childRole: 'Afya ya Mtoto',
    careTeamRole: 'Wahudumu wa Afya',
    milestones: {
      pregnancy: {
        title: 'Ujauzito',
        desc: 'Miongozo ya miezi mitatu, dalili & maandalizi',
      },
      anc: {
        title: 'Kliniki ya ANC',
        desc: 'Ufuatiliaji wa ziara za MOH 216, rekodi za kliniki & maandalizi ya huduma',
      },
      birth: {
        title: 'Kujifungua',
        desc: 'Maandalizi ya uzazi & mpango wa kujifungua',
      },
      newborn: {
        title: 'Mtoto Mchanga',
        desc: 'Unyonyeshaji, huduma ya kitovu, dalili za hatari & huduma za awali',
      },
      childHealth: {
        title: 'Afya ya Mtoto',
        desc: 'Chanjo, ufuatiliaji wa magonjwa & huduma za kinga',
      },
      growth: {
        title: 'Ukuaji & Hatua',
        desc: 'Uzito, urefu, maendeleo & hatua za ukuaji',
      },
      ongoing: {
        title: 'Huduma Endelevu',
        desc: 'Ushirikiano wa familia na wahudumu wa afya',
      },
      reminders: {
        title: 'Vikumbusho vya Moja kwa Moja',
        desc: 'Miadi muhimu, hatua za ukuaji, kazi za afya na ufuatiliaji vinabaki wazi vinapohitajika.',
      },
    },
    steps: {
      pregnancy: {
        title: 'Ujauzito',
        desc: 'Miongozo ya miezi mitatu, dalili & maandalizi',
        focus: 'Afya ya Mama',
      },
      anc: {
        title: 'Kliniki ya ANC',
        desc: 'Ufuatiliaji wa ziara za MOH 216, rekodi za kliniki & maandalizi ya huduma',
        focus: 'Wahudumu wa Afya',
      },
      birth: {
        title: 'Kujifungua',
        desc: 'Maandalizi ya uzazi & mpango wa kujifungua',
        focus: 'Mama na Familia',
      },
      newborn: {
        title: 'Mtoto Mchanga',
        desc: 'Unyonyeshaji, huduma ya kitovu, dalili za hatari & huduma za awali',
        focus: 'Afya ya Mtoto',
      },
      childHealth: {
        title: 'Afya ya Mtoto',
        desc: 'Chanjo, ufuatiliaji wa magonjwa & huduma za kinga',
        focus: 'Mtoto & Kliniki',
      },
      growth: {
        title: 'Ukuaji & Hatua',
        desc: 'Uzito, urefu, maendeleo & hatua za ukuaji',
        focus: 'Afya ya Mtoto',
      },
      ongoing: {
        title: 'Huduma Endelevu',
        desc: 'Ushirikiano wa familia na wahudumu wa afya',
        focus: 'Mama & Mtoto',
      },
    },
  },

  havenShowcase: {
    badge: 'Mwenzi wa Afya Mwenye Majibu ya Haraka',
    heading: 'Maswali ya afya hayasubiri hadi siku ya miadi.',
    companionTitle: 'Kutana na Haven, mshauri wako wa afya ndani ya MomHaven.',
    description: 'Uliza maswali kuhusu ujauzito, uzazi, na malezi ya mtoto. Haven inakusaidia kuelewa taarifa za afya, kupata mwongozo sahihi na kutambua wakati huduma ya kliniki inahitajika.',
    tryHaven: 'Jaribu Mazungumzo ya Haven',
    readyStatus: 'Imejengwa na Mwongozo wa MOH Kenya • Tayari Kusaidia',
    sampleQuestion: 'Je, mgandamizo wa chini ya tumbo katika wiki ya 24 ni kawaida?',
    sampleAnswer: 'Mgandamizo mdogo wa chini ya tumbo ni jambo la kawaida katika robo ya pili mtoto anapokua na misuli ya uzazi inapovutika. Hata hivyo, ni muhimu kufuatilia dalili za hatari.',
    sampleDietTip: 'Kidokezo: Kula vyakula vya kienyeji vyenye madini ya chuma kama sukuma wiki, managu, terere, na maharage pamoja na matunda ya vitamin C kuongeza nguvu.',
    sampleWarning: '⚠️ Ikiwa mgandamizo unaambatana na maumivu makali ya tumbo, maji kutoka, homa, au damu, nenda kituo cha afya mara moja.',
    sampleAskOwn: 'Muulize Haven swali lako mwenyewe →',
    suggestedChips: [
      'Dalili zipi za hatari wakati wa ujauzito?',
      'Ratiba ya chanjo za KEPI kwa mtoto',
      'Vyakula vinavyoongeza maziwa ya mama',
    ],
  },

  knowledge: {
    heading: 'Jua nini cha kutarajia. Jua nini cha kuuliza.',
    subheading: 'Miongozo ya kitabibu iliyorahisishwa na kueleweka vyema kwa kila hatua.',
    categories: {
      pregnancy: {
        title: 'Ujauzito',
        desc: 'Mabadiliko gani hutokea kila robo ya mwaka? Ujiandaeje?',
        tag: 'Miongozo ya Robo 1, 2, 3',
      },
      newborn: {
        title: 'Malezi ya mchanga',
        desc: 'Unyonyeshaji, usingizi, dalili za hatari, chanjo na huduma za mapema.',
        tag: 'Mwongozo wa Miezi 0–6',
      },
      childHealth: {
        title: 'Afya ya mtoto',
        desc: 'Ukuaji, maendeleo, lishe bora na kinga ya maradhi.',
        tag: 'Chanjo za EPI & Lishe',
      },
      motherHealth: {
        title: 'Afya ya mama',
        desc: 'Kupona baada ya uzazi, afya ya akili na wakati wa kutafuta msaada.',
        tag: 'Ustawi Baada ya Kujifungua',
      },
    },
    cta: 'Gundua maktaba ya maarifa ya MomHaven',
  },

  trust: {
    badge: 'Uadilifu wa Kitabibu & Usalama',
    heading: 'Imejengwa kwa msingi wa huduma zinazoaminika.',
    subheading: 'Imeundwa kuimarisha uelewa wa mama na kurahisisha mashauriano kwenye vituo vya afya.',
    pillars: {
      evidence: {
        title: 'Inayozingatia ushahidi wa kitabibu',
        desc: 'Taarifa za afya zinazoendana na miongozo ya Wizara ya Afya Kenya (MOH 216) na viwango vya kimataifa vya afya ya uzazi.',
      },
      privacy: {
        title: 'Inayojali faragha yako',
        desc: 'Taarifa zako za afya zinashughulikiwa kwa heshima ya faragha na ridhaa kamili, zikiwa wazi kwa wale tu unaochagua kuwashirikisha.',
      },
      care: {
        title: 'Imebuniwa kwa huduma halisi',
        desc: 'MomHaven inamsaidia mama kujiandaa na kushiriki kikamilifu mazungumzo na daktari, akiwa na vipimo na historia yake mkononi.',
      },
    },
  },

  clinician: {
    badge: 'Daraja la Kuelekea Kliniki',
    heading: 'Taarifa zako zinasafiri nawe popote unapoenda.',
    description: 'Ukiwa tayari, unaweza kushiriki taarifa zako za afya kwa usalama na mhudumu wako wa afya—ili mambo muhimu yasibaki kwenye kumbukumbu ya kichwa pekee.',
    portalButton: 'Ingia Mlango wa Wahudumu wa Afya',
    learnMore: 'Jifunze kuhusu kushiriki na daktari',
  },

  partner: {
    badge: 'Ushirika wa Malezi',
    heading: 'Safari ya uzazi haipaswi kubebwa peke yako.',
    description: 'Mwalike mtu unayemwamini akusaidie kwenye safari hii. Wenza wanaweza kufuatilia hatua za ujauzito, tarehe za kliniki, na vidokezo vya kusaidia kupunguza mzigo wa mawazo.',
    cta: 'Namsaidia mtu mwingine',
    howItWorks: 'Jinsi usaidizi wa mwenzi unavyofanya kazi',
    cardTitle: 'Kuunganisha na Mwenzi',
    cardDesc: 'Shiriki tarehe za kliniki, mawasiliano ya dharura, na hatua za kila siku na mwenzi unayemwamini.',
    cardCheck: '✓ Kalenda ya pamoja & mawasiliano ya dharura',
  },

  accountCta: {
    heading: 'Uko tayari kuifanya MomHaven kuwa yako?',
    description: 'Fungua akaunti ili kuhifadhi safari yako ya afya, kurekodi vipimo, na kubeba taarifa zako muhimu popote uendapo.',
    createButton: 'Fungua akaunti yako sasa',
    signInButton: 'Tayari una akaunti? Ingia',
  },

  footer: {
    about: 'Kuhusu MomHaven',
    privacy: 'Faragha & Ulinzi wa Data',
    safety: 'Miongozo ya Usalama',
    clinicianPortal: 'Mlango wa Wahudumu wa Afya',
    partnerAccess: 'Mlango wa Wenza',
    copyright: 'MomHaven • Mwenzi wa Kidijitali wa MOH 216 Kenya',
  },

  modals: {
    about: {
      title: 'Kuhusu MomHaven',
      subtitle: 'Kuwezesha akina mama na watoto kwa mwongozo wa afya wa kidijitali wenye hadhi na ushahidi wa kitabibu.',
      missionTitle: 'Dhamira Yetu',
      missionText: 'MomHaven imeundwa kumuunga mkono kila mama na mtoto katika kila hatua muhimu—kuanzia wiki za kwanza za ujauzito, uzazi salama, malezi ya mtoto mchanga, chanjo, na ukuaji wa mapema.',
      mohTitle: 'Ushirikiano na Miongozo ya MOH 216',
      mohText: 'Tunahamisha maudhui na mifumo ya Kitabu cha Afya ya Mama na Mtoto (MOH 216) cha Wizara ya Afya Kenya kuwa mfumo rahisi wa kidijitali unaoweka vipimo, ratiba za kliniki, na miongozo katika sehemu moja.',
      philosophyTitle: 'Falsafa Yetu ya Huduma',
      philosophyText: 'Tunaamini zana za afya zinapaswa kuwawezesha wanawake, kuheshimu mazingira na mila zetu, na kurahisisha mashauriano yenye tija kati ya mama na wahudumu wa afya walioidhinishwa.',
    },
    privacy: {
      title: 'Faragha & Ulinzi wa Data',
      subtitle: 'Ushughulikiaji wa rekodi za afya ya mama na mtoto kwa uwazi na ridhaa kamili.',
      principlesTitle: 'Ahadi Zetu Kuu za Faragha',
      item1Title: 'Wewe Ndiye Mmiliki wa Data Yako',
      item1Text: 'Rekodi zako za afya, maelezo, na hatua za maendeleo ni mali yako pekee. MomHaven haiuzi, haitoi udalali, wala kutumia data yako ya afya kwa biashara.',
      item2Title: 'Ridhaa ya Kina ya Kushiriki',
      item2Text: 'Wewe ndiye unayeamua kumuunganisha mwenzi au daktari. Ushirikiano huu unaweza kusitishwa wakati wowote kwa kubadilisha nambari za uunganishaji.',
      item3Title: 'Usalama na Usimbaji Fiche',
      item3Text: 'Vipimo vyote vya afya, miadi ya kliniki, na mazungumzo yanasafirishwa kwa itifaki zilizolindwa na kuhifadhiwa kwa usalama wa hali ya juu.',
    },
    safety: {
      title: 'Miongozo ya Usalama & Mipaka ya Kitabibu',
      subtitle: 'Kuelewa wajibu wa MomHaven pamoja na huduma za kitaalamu za madaktari.',
      disclaimerTitle: 'Mwongozo wa Kuelimisha, Sio Utambuzi wa Kimatibabu',
      disclaimerText: 'MomHaven na mshauri Haven hutoa elimu ya afya inayozingatia miongozo ya MOH Kenya. Haichukui nafasi ya uchunguzi wa kimwili wa daktari, vipimo vya maabara, au matibabu rasmi.',
      dangerSignsTitle: 'Dalili za Hatari za Dharura kwa Mama na Mtoto Mchanga',
      dangerSignsList: [
        'Kutokwa na damu ukeni au maji mengi wakati wa ujauzito',
        'Maumivu makali ya kichwa, macho kuona giza, au kuvimba uso na mikono',
        'Homa kali, kutetemeka, au uchafu wenye harufu mbaya ukeni',
        'Mtoto kuacha au kupunguza kucheza tumboni katika miezi ya mwisho',
        'Mtoto mchanga kushindwa kupumua vizuri, degedege, au kushindwa kunyonya',
      ],
      emergencyAction: 'Ukiona dalili yoyote kati ya hizi, nenda mara moja kwenye kituo cha afya kilicho karibu au piga simu za dharura 1199 / 999.',
    },
  },

  auth: {
    welcomeBack: 'Karibu tena',
    createAccount: 'Fungua akaunti yako',
    passwordlessLink: 'Kiungo cha Kuingia Bila Nenosiri',
    resetPassword: 'Badilisha nenosiri lako',
    welcomeSubtitle: 'Safari yako ya huduma, yote sehemu moja.',
    createSubtitle: 'Hifadhi hatua zako, rekodi za afya, na mazungumzo na Haven.',
    magicSubtitle: 'Ingia kwa usalama kupitia kiungo kitakachotumwa kwenye barua pepe yako.',
    resetSubtitle: 'Weka barua pepe yako ili kupokea maagizo ya kubadilisha nenosiri.',
    continueWithGoogle: 'Endelea na Google',
    continueWithEmail: 'Endelea na barua pepe',
    fullName: 'Jina lako (Sio lazima)',
    emailAddress: 'Barua Pepe',
    password: 'Nenosiri',
    forgotPassword: 'Umesahau nenosiri?',
    sendMagicLink: 'Tuma Kiungo cha Kuingia',
    sendResetLink: 'Tuma Kiungo cha Kubadilisha',
    useMagicLinkInstead: 'Tumia kiungo cha barua pepe badala yake',
    signInWithPassword: 'Ingia kwa kutumia nenosiri',
    alreadyHaveAccount: 'Tayari una akaunti?',
    dontHaveAccount: 'Huna akaunti bado?',
    supportingSomeone: 'Je, unamsaidia mama mjamzito?',
    supportingSomeoneDesc: 'Jiunge na MomHaven kama mwenzi ufuatilie safari yake kwa ukaribu.',
    exploreWithoutAccount: 'Gundua MomHaven bila kufungua akaunti',
    healthcareProfessional: 'Wewe ni mhudumu wa afya?',
    clinicianPortal: 'Mlango wa Wahudumu',
    checkInbox: 'Angalia kikasha chako cha barua pepe!',
    magicLinkSentMessage: 'Tumetuma kiungo cha kuingilia kwenye barua pepe',
    resetSentTitle: 'Maagizo Yatumwa',
    resetSentMessage: 'Tumetuma maagizo ya kubadilisha nenosiri kwa',
    backToSignIn: 'Rudi Kwenye Kuingia',
    backToAllOptions: 'Rudi kwenye chaguo zote',
  },

  modules: {
    today: {
      greeting: 'Habari',
      gestationalWeek: 'Wiki ya',
      trimester: 'Robo ya',
      daysUntilEdd: 'siku zilizosalia kabla ya kujifungua',
      logWellbeing: 'Rekodi Hali Yako ya Leo',
      askHaven: 'Muulize Haven AI',
      emergencyAction: 'Dalili za Hatari & Usalama wa Haraka',
      upcomingClinic: 'Kliniki Zinazokuja',
      recentEncounters: 'Rekodi za Hivi Karibuni za Kliniki',
      quickActions: 'Hatua za Haraka',
      babySize: 'Ulinganisho wa Ukubwa wa Mtoto',
      readinessTitle: 'Mpango wa Kujifungua Salama',
    },
    anc: {
      title: 'Daftari la Huduma ya Wajawazito (ANC)',
      subTitle: 'Mfumo wa Ziara 8 za WHO & MOH',
      handbookBanner: 'JAMHURI YA KENYA · WIZARA YA AFYA',
      handbookCode: 'MOH 216 · SEHEMU YA 2',
      adherenceTracker: 'Uzingatiaji wa Kliniki',
      contactsLogged: 'kati ya Ziara 8 Zilizorekodiwa',
      contactsRegistry: 'Orodha ya Ziara za Kliniki',
      recordContact: 'Rekodi Ziara ya Kliniki (ANC)',
      facility: 'Kituo cha Afya',
      bloodPressure: 'Shinikizo la Damu',
      fundalHeight: 'Kimo cha Mji wa Mimba',
      fetalHeartRate: 'Mapigo ya Moyo wa Mtoto',
      weight: 'Uzito',
      ironFolic: 'Vidonge vya Madini ya Chuma (IFAS)',
      tdBooster: 'Chanjo ya Pepopunda (Td Booster)',
      iptpMalaria: 'Kinga ya Malaria Wakati wa Ujauzito (IPTp)',
      mosquitoNet: 'Chandarua chenye Dawa (LLIN)',
    },
    immunization: {
      title: 'Daftari la Chanjo za Mtoto',
      subTitle: 'Mwongozo wa KEPI (kurasa 31–34)',
      handbookCode: 'MOH 216 · PASIPOTI YA CHANJO YA KEPI',
      dosesRecorded: 'kati ya Chanjo {total} Zilizotolewa',
      verified: 'Imethibitishwa na Daktari',
      recordVaccine: 'Rekodi Chanjo Iliyotolewa',
      filterAll: 'Zote',
      filterGiven: 'Zilizotolewa',
      filterPending: 'Zinazosubiriwa',
      route: 'Njia ya Kutoa',
      targetDisease: 'Inakinga dhidi ya',
      administeredOn: 'Tarehe ya Kutolewa',
      batchNumber: 'Nambari ya Kundi (Batch)',
    },
    growth: {
      title: 'Viwango vya Ukuaji vya WHO (0–60m)',
      subTitle: 'Kitabu cha Afya cha MOH Kenya kurasa 28–35',
      handbookCode: 'MOH 216 · UKUAJI NA LISHE YA MTOTO',
      currentTrajectory: 'Mwenendo wa Sasa & Kizio cha Z-Score',
      weightTab: 'Uzito (0–60m)',
      heightTab: 'Urefu',
      muacTab: 'Utepe wa MUAC',
      headTab: 'Mzunguko wa Kichwa',
      recordMeasurement: 'Rekodi Kipimo',
      monthsOld: 'umri wa miezi',
      feedingStatus: 'Hali ya Unyonyeshaji',
      exclusiveBreastfeeding: 'Unyonyeshaji wa Maziwa ya Mama Pekee',
    },
    offline: {
      offlineMode: 'Hali Isiyo na Mtandao Inaendelea',
      offlineNotice: 'Mabadiliko yamehifadhiwa kwenye simu na yatasawazishwa mtandao ukirejea.',
      pendingWrites: 'rekodi zinasubiri kusawazishwa mtandaoni',
      syncing: 'Inasawazisha rekodi za bila mtandao...',
      allSynced: 'Rekodi zote zimesawazishwa kikamilifu',
      reconnected: 'Mtandao umerejea — inasawazisha mabadiliko yako sasa',
    },
    pnc: {
      title: 'Daftari la Huduma Baada ya Kujifungua (PNC)',
      subTitle: 'Mwongozo wa Kliniki wa MOH 216 (uk. 20)',
      handbookCode: 'MOH 216 · SEHEMU YA 3',
      motherSection: 'Sehemu A: Uchunguzi wa Mama',
      babySection: 'Sehemu B: Uchunguzi wa Mtoto',
      clinicalActions: 'Sehemu C: Matibabu & Hatua za Kliniki',
      contacts: {
        h48: 'Ziara ya 1: Ndani ya Saa 48',
        w1_2: 'Ziara ya 2: Wiki 1–2',
        w4_6: 'Ziara ya 3: Wiki 4–6',
        m4_6: 'Ziara ya 4: Miezi 4–6',
      },
      fields: {
        bloodPressure: 'Shinikizo la Damu',
        temperature: 'Joto la Mwili',
        pulse: 'Mapigo ya Moyo',
        respiratoryRate: 'Kupumua',
        uterineInvolution: 'Kupona kwa Mji wa Mimba',
        involutionContracted: 'Umesinyaa Vizuri (Kawaida)',
        involutionWell: 'Umerudi Kawaida (Salama)',
        involutionSubinvoluted: 'Kuchelewa Kurudi Kawaida',
        involutionTender: 'Inauma (Inahitaji Uangalizi wa Daktari)',
        lochia: 'Kutokwa na Damu Baada ya Kujifungua (Lochia)',
        lochiaAmount: 'Kiasi cha Damu',
        lochiaColour: 'Rangi ya Damu',
        lochiaSmell: 'Harufu ya Damu',
        lochiaNormal: 'Kawaida',
        lochiaFoul: 'Harufu Mbaya (Onyo la Maambukizi)',
        haemoglobin: 'Kiwango cha Damu (Hb)',
        hivRetest: 'Kipimo cha Afya cha Virusi vya UKIMWI',
        onHaart: 'Matumizi ya Dawa za HAART',
        familyPlanning: 'Ushauri wa Uzazi wa Mpango',
        fpMethod: 'Njia Iliyochaguliwa',
        mentalHealth: 'Hali ya Kihisia na Afya ya Akili ya Mama',
        mentalHealthNoConcerns: 'Hali Nzuri (Hakuna Shaka)',
        mentalHealthConcerns: 'Dalili Zimeonekana (Msaada Unahitajika)',
        mentalHealthReferred: 'Amepewa Rufaa kwa Msaada Maalum',
        babyTemp: 'Joto la Mwili wa Mtoto',
        babyWeight: 'Uzito wa Mtoto',
        feedingMethod: 'Njia ya Kulisha Mtoto',
        ebf: 'Kunyonyesha Maziwa ya Mama Pekee (EBF)',
        replacement: 'Maziwa Mbadala',
        mixed: 'Mchanganyiko wa Maziwa',
        cordCondition: 'Kupona kwa Kitovu',
        cordClean: 'Safi na Kikavu (Kawaida)',
        cordMoist: 'Kinamaji / Kuchelewa Kukauka',
        cordInfected: 'Wekundu / Usaha (Unahitaji Daktari Mara Moja)',
        eyeCondition: 'Uchunguzi wa Macho',
        eyeNormal: 'Macho Masafi (Kawaida)',
        eyeSticky: 'Macho Yenye Usaha / Uchafu',
        breastCondition: 'Hali ya Matiti',
        breastNormal: 'Laini na Kawaida',
        breastEngorged: 'Yamejaa Maziwa',
        breastMastitis: 'Yana Maumivu Makali / Uvimbe',
      },
    },
    td: {
      title: 'Kinga ya Pepopunda na Dondakoo kwa Mama (Td)',
      handbookCode: 'MOH 216 · TAARIFA ZA UJAUZITO',
      status: 'Hali ya Kinga ya Sasa',
      dosesReceived: 'Dozi Zilizopokelewa',
      nextDue: 'Dozi Inayofuata',
      lifelongProtected: 'Kinga Kamili ya Maisha Yote Imekamilika (Dozi 5)',
      schedule: 'Mfumo wa Dozi 5 wa WHO/MOH',
      restartWarning: 'Hakuna haja ya kuanza upya — kila dozi iliyopokelewa inazingatiwa',
    },
    referrals: {
      referredBadge: 'Imeelekezwa — inasubiri ufuatiliaji',
      openReferralTitle: 'Rufaa ya Afya Inayoendelea',
      awaitingFollowUp: 'Inasubiri Ufuatiliaji wa Kliniki',
      reason: 'Sababu ya Rufaa',
      priority: 'Kiwango cha Kipaumbele',
      urgentFlag: 'Uangalizi wa Haraka wa Matibabu',
      targetFacility: 'Kituo Kilichoelekezwa',
      serviceNeeded: 'Huduma Inayohitajika',
      clinicalNotes: 'Maelezo ya Rufaa',
      motherGuidance: 'Mhudumu wako wa afya amerekodi uhitaji wa ufuatiliaji wa haraka au maalum. Tafadhali hudhuria miadi yako ya rufaa au zungumza na kliniki yako mapema.',
    },
  },
};
