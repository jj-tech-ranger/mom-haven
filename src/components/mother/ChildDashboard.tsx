...
export const ChildDashboard: React.FC<ChildDashboardProps> = ({ child, childrenList = [], onSwitchChild, onOpenNewbornRecord, onOpenPncOverview, onOpenImmunization, onOpenGrowth, onOpenDevelopment, onOpenTimeline }) => {
  ...
  const [counts, setCounts] = useState({ immunization: 0, growth: 0, development: 0 });

  useEffect(() => { setNameDraft(child?.name || ''); setNameError(''); }, [child?.id, child?.name]);
  useEffect(() => {
    if (!child?.id) { setCounts({ immunization: 0, growth: 0, development: 0 }); return; }
    const mappings = [
      ['immunizationRecords', 'immunization'],
      ['growthMeasurements', 'growth'],
      ['developmentRecords', 'development'],
    ] as const;
    const unsubs = mappings.map(([subcollection, key]) => onSnapshot(collection(db, 'children', child.id, subcollection), (snapshot) => setCounts((current) => ({ ...current, [key]: snapshot.size })), (error) => console.error(`Could not load ${subcollection}:`, error)));
  ...
  return <div className="min-h-screen bg-lavender-50 pb-24">
    ...
    <main className="p-4 space-y-4 max-w-lg mx-auto">
      ...
      <div className="grid grid-cols-3 gap-2.5">{statuses.map(({ label, count, icon: Icon, action }) => <button key={label} onClick={action} className="bg-white rounded-[20px] border border-border-hairline shadow-card-1 p-3 text-center"><Icon className="w-5 h-5 text-haven-orchid mx-auto mb-2" /><span className="font-display font-bold text-xs text-ink-900 block">{count ? `${count} recorded` : 'Not started'}</span><span className="font-body text-[10px] text-ink-600">{label}</span></button>)}</div>
