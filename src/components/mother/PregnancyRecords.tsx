...
import { ChevronLeft, Heart, Calendar, ChevronRight, Plus } from 'lucide-react';
...
export const PregnancyRecords: React.FC<PregnancyRecordsProps> = ({
  pregnancy,
  onBack,
  onSelectRecord,
  onAddRecord,
}) => {
  const currentWeek = pregnancy?.gestationalAgeWeeks || 24;
  const progressPercent = Math.min(100, Math.round((currentWeek / 40) * 100));

  const sampleRecords = [
    {
      id: 'preg_rec_1',
      title: 'ANC Contact 4 · Clinical Review',
      date: '2 Mar 2026',
      provenance: {
      ...
      summary: 'Blood pressure 112/74, Fundal height 24cm, FHR 144bpm',
    },
    {
      id: 'preg_rec_2',
      title: 'ANC Contact 3 · Caregiver Log',
      date: '1 Feb 2026',
      provenance: {
      ...
      summary: 'Tetanus toxoid 2nd dose, IFAS refill received',
    },
    {
      id: 'preg_rec_3',
      title: 'First Ultrasound Scan',
      date: '10 Nov 2025',
      provenance: {
        status: 'VERIFIED' as const,
        enteredBy: 'dr_1',
        enteredAt: '2025-11-10',
        verifiedBy: 'Dr. O. Kiprono',
        verifiedAt: '2025-11-10',
        facilityName: 'Pumwani Maternity Hospital',
      },
      summary: 'Single live intrauterine fetus, gestational age 12w 3d',
    },
  ];
  ...
  return (
    <div className="space-y-6 pb-12 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        ...
        <h1 className="font-display font-bold text-xl text-ink-900">Pregnancy Records</h1>
        <div className="w-10" />
      </div>

      {/* Haven Ribbon Gestational Progress */}
      <div className="bg-white rounded-[20px] border border-border-hairline shadow-card-1 p-5 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-display font-bold text-haven-deep">
            Week {currentWeek} of 40
          </span>
        </div>
        <HavenRibbon progress={progressPercent} currentStep={currentWeek} totalSteps={40} />
      </div>
      ...
      <div className="space-y-3">
        ...
        {sampleRecords.length === 0 ? (
          <EmptyState
            icon={Heart}
            title="No pregnancy records yet"
            message="Your ANC visits and ultrasound scans will appear here once logged."
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="font-body text-xs text-ink-600 mt-0.5">{r.date}</p>
                  </div>
