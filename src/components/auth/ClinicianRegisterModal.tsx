...
import { createAccountWithEmail, signInWithEmail } from '../../lib/firebase';
...
type Mode = 'login' | 'register' | 'submitted' | 'status' | 'setup';
...
export default function ClinicianRegisterModal({ onClose, onSuccess }: ClinicianRegisterModalProps) {
  ...
  useEffect(() => {
    let mounted = true;
    void getKenyaFacilities()
      .then(result => { if (mounted) setFacilities(result); })
      .catch(err => { console.error(err); if (mounted) setError('Unable to load the Kenya facility directory. Please try again.'); })
  ...
  const handleClinicianLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim() || !password) { setError('Enter your professional email and clinician password.'); return; }
    try {
      setLoading(true);
      setError(null);
      ...
      console.error('Clinician sign-in error', err);
      if (err?.code === 'auth/wrong-password' || err?.code === 'auth/user-not-found' || err?.code === 'auth/invalid-credential') {
        setError('Invalid clinician email or password. If your application was only recently approved, use “Check verification status” to activate your account first.');
      } else if (err?.code === 'auth/too-many-requests') {
        setError('Too many sign-in attempts. Please wait a moment and try again.');
      } else {
        setError(err?.message || 'Unable to sign in to the clinician portal. Please try again.');
      }
  ...
  const handleCreatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err?.code === 'auth/email-already-in-use') setError('An account already exists for this email. Use the clinician sign-in screen instead.');
  ...
  const renderHeader = () => (
    <div className="p-5 border-b border-[var(--border-hairline)] flex items-center justify-between">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="font-display font-extrabold text-[18px] text-[var(--ink-900)]">Healthcare Professional Access</h3>
          <p className="font-body text-xs text-[var(--ink-600)]">KMPDC / NCK / COC Credential Verification</p>
        </div>
      </div>
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          ...
          {mode === 'login' && <>
            <div className="bg-[var(--lavender-50)] border border-[var(--border-hairline)] p-3.5 rounded-[16px] text-xs text-[var(--ink-900)] flex items-start gap-2.5">
              ...
              <span><strong>Registered clinicians:</strong> Sign in here with the professional email and password created after administrator approval.</span>
            </div>
            <form onSubmit={handleClinicianLogin} className="space-y-3.5 text-left">
              <input type="email" placeholder="Professional email" value={email} onChange={e => setEmail(e.target.value)} required autoFocus className="w-full text-xs py-3 px-3.5 rounded-[14px] border border-[var(--border-hairline)] bg-[var(--lavender-50)]" />
              <div className="relative">
                <LockKeyhole className="absolute left-3 top-3 w-4 h-4 text-[var(--ink-500)]" />
                <input type="password" placeholder="Clinician password" value={password} onChange={e => setPassword(e.target.value)} required className="w-full text-xs py-3 pl-9 pr-3.5 rounded-[14px] border border-[var(--border-hairline)] bg-[var(--lavender-50)]" />
              </div>
              <Button type="submit" variant="primary" disabled={loading} className="w-full py-3.5 text-xs font-display font-bold">{loading ? 'Signing in...' : 'Sign in to Clinician Portal'}</Button>
            </form>
            <div className="flex flex-col gap-2 text-center">
              <button type="button" onClick={() => { setError(null); setMode('status'); }} className="text-xs font-display font-bold text-[var(--haven-deep)] hover:underline cursor-pointer">Approved but need to activate your account? Check verification status</button>
              <button type="button" onClick={() => { setError(null); setPassword(''); setMode('register'); }} className="text-xs font-display font-semibold text-[var(--ink-600)] hover:text-[var(--haven-deep)] hover:underline cursor-pointer">New clinician? Submit a verification request</button>
            </div>
          </>}
