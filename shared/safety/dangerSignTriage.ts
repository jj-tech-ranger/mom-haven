...
export interface DangerSign{id:string;label:string;icon:string;category:DangerSignCategory;matchPatterns:string[]}
...
export const ADDITIONAL_PHYSICAL_DANGER_PATTERNS:RegExp[]=[/\bblurred vision\b/i,/\bcan't breathe\b/i,/\bnot breathing\b/i,/\bfast or difficult breathing\b/i];
export const DANGER_SIGN_PATTERNS:RegExp[]=[...DANGER_SIGNS.flatMap(sign=>sign.matchPatterns).map(pattern=>new RegExp(`\\b${escapeRegExp(pattern).replace(/\\s+/g,'\\s+')}\\b`,'i')),...ADDITIONAL_PHYSICAL_DANGER_PATTERNS];
export const SELF_HARM_OR_VIOLENCE_PATTERNS:RegExp[]=[...DANGER_SIGNS.filter(sign=>sign.category==='selfharm').flatMap(sign=>sign.matchPatterns).map(pattern=>new RegExp(`\\b${escapeRegExp(pattern).replace(/\\s+/g,'\\s+')}\\b`,'i')),/\b(?:being|been) (?:hit|beaten|hurt|assaulted)\b/i,/\b(?:partner|husband|someone) (?:hits|hit|beats|beat|hurts|hurt) me\b/i,/\bdomestic violence\b/i,/\bsexual assault\b/i,/\brape(?:d)?\b/i,/\babuse(?:d)?\b/i];
...
export function classifyLayerOne(message:string):LayerOneResult{const normalized=message.replace(/\s+/g,' ').trim();if(SELF_HARM_OR_VIOLENCE_PATTERNS.some(pattern=>pattern.test(normalized)))return'self_harm_or_violence';if(DANGER_SIGN_PATTERNS.some(pattern=>pattern.test(normalized)))return'physical_danger';return null}
export function matchesPositivePregnancyTestContext(message:string):boolean{return POSITIVE_TEST_CONTEXT.test(message.replace(/\s+/g,' ').trim())}
